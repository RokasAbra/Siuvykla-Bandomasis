...
function Edit() {
  const { modalProduct, setModalProduct, setEditProduct, categories, setDeletetePhoto } =
  ...
  useEffect(() => {
    ...
    setCat(categories.filter((c) => c.title === modalProduct.cat)[0].id);
    setLu(setDateFormat(modalProduct.lu));
    setPhotoPrint(modalProduct.photo);
  }, [modalProduct, categories]);
